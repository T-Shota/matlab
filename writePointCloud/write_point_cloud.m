...
load(dataFile);
ptCloudRef = livingRoomData{10};

%% create a publisher
rosshutdown;
rosinit;
plane_pub = rospublisher('/plane_detection_output','sensor_msgs/PointCloud2');

%% create "sensor_msgs/PointCloud2" message
ptMsg = rosmessage('sensor_msgs/PointCloud2');
ptMsg.Header = rosmessage('std_msgs/Header');
ptMsg.Height = size(ptCloudRef.Location,1);
ptMsg.Width = size(ptCloudRef.Location,2);
ptMsg.PointStep = 32;
...
ptMsg.Fields(4).Name = 'rgb';
ptMsg.Fields(4).Offset = 16;
ptMsg.Fields(4).Datatype = 7;
ptMsg.Fields(4).Count = 1;

locs = reshape(ptCloudRef.Location,[],3);
colors = uint32(reshape(ptCloudRef.Color,[],3));
rgb = typecast(colors(:,1)*2^16+colors(:,2)*2^8+colors(:,3),'single');
data = [locs zeros(size(locs,1),1,'single') rgb zeros(size(locs,1),3,'single')];
